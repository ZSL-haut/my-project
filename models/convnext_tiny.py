...
try:
    ...
    HAS_TIMM = False
    print("⚠️  timm library not found. Please install it: pip install timm")
...
class Config:
    ...
    feature_dim = 768
    patience = 20
    output_dir = '../results/result1/convnext_tiny'
    local_convnext_pretrained_path = '../pre_models/convnext_tiny.bin'
...
def load_convnext_pretrained_weights(model, weight_path):
    """
    加载 ConvNeXt-Tiny 预训练权重的专用函数
    支持 .bin 和 .pth 格式，处理键名不匹配问题

    Args:
        model: timm 的 ConvNeXt-Tiny 模型实例
        weight_path: 权重文件路径（.bin 或 .pth）

    Returns:
        model: 加载权重后的模型
    """
    print(f"\n{'=' * 100}")
    print(f"正在加载 ConvNeXt-Tiny 预训练权重...")
    print(f"权重文件路径: {weight_path}")
    print(f"文件格式: {os.path.splitext(weight_path)[1]}")
    print(f"{'=' * 100}")
    ...
    try:
        ...
        if isinstance(checkpoint, dict):
            # 尝试常见的键名
            possible_keys = ['model', 'state_dict', 'model_state', 'net', 'model_ema']
            state_dict = None
        ...
        # 3. 处理键名前缀问题（timm 模型可能有 'model.' 前缀）
        # 检查是否需要移除或添加前缀
        model_keys = set(model.state_dict().keys())
        state_keys = set(state_dict.keys())

        # 采样检查键名格式
        sample_model_key = list(model_keys)[0] if model_keys else ""
        sample_state_key = list(state_keys)[0] if state_keys else ""

        print(f"\n键名格式检查:")
        print(f"  - 模型键示例: {sample_model_key}")
        print(f"  - 权重键示例: {sample_state_key}")

        # 如果权重有 'model.' 前缀但模型没有，则移除
        if sample_state_key.startswith('model.') and not sample_model_key.startswith('model.'):
            print(f"  - 检测到权重文件有 'model.' 前缀，正在移除...")
            new_state_dict = {}
        ...
        # 如果模型有 'model.' 前缀但权重没有，则添加
        elif sample_model_key.startswith('model.') and not sample_state_key.startswith('model.'):
            print(f"  - 检测到模型有 'model.' 前缀，正在添加...")
            new_state_dict = {}
            for k, v in state_dict.items():
        ...
        model_keys_list = list(model_state_dict.keys())
        print(f"\n模型信息:")
        print(f"  - 模型参数数量: {len(model_keys_list)}")
        print(f"  - 示例键名: {model_keys_list[:5]}")
        ...
        # 7. 统计匹配情况
        total_keys = len(model_keys_list)
        matched_keys = total_keys - len(missing_keys)
        match_ratio = (matched_keys / total_keys) * 100 if total_keys > 0 else 0
        ...
        if match_ratio >= 95:
            ...
            print(f"   大部分权重已加载，少数层（如分类头）使用随机初始化")
        elif match_ratio >= 50:
            print(f"\n⚠️  警告：预训练权重部分加载，匹配率: {match_ratio:.2f}%")
            print(f"   约一半的权重被加载，建议检查权重文件是否正确")
        else:
            print(f"\n❌ 错误：权重匹配率过低 ({match_ratio:.2f}%)")
            print(f"   权重文件可能不兼容，建议：")
            print(f"   1. 检查权重文件是否是 ConvNeXt-Tiny")
            print(f"   2. 确认权重来源（timm/torchvision/其他）")
            print(f"   3. 考虑使用 timm 官方预训练权重")
...
# 改进的多模态 ConvNeXt 模型
class ImprovedMultimodalConvNeXt(nn.Module):
    def __init__(self, num_classes, pretrained_path=None):
        super().__init__()

        if not HAS_TIMM:
            raise ImportError(
                "timm library is required for ConvNeXt-Tiny. "
                "Please install it: pip install timm"
            )

        print(f"\n{'=' * 100}")
        print("初始化 ConvNeXt-Tiny 多模态模型")
        print(f"{'=' * 100}\n")

        # 1. 创建 ConvNeXt-Tiny 模型（不加载预训练权重）
        print("创建 ConvNeXt-Tiny 模型结构...")
        self.img_convnext = timm.create_model('convnext_tiny', pretrained=False)
        print("✓ 模型结构创建成功")

        # 2. 尝试加载本地预训练权重
        weights_loaded = False
        if pretrained_path and os.path.exists(pretrained_path):
            result = load_convnext_pretrained_weights(self.img_convnext, pretrained_path)
        ...
        if not weights_loaded:
            ...
            print("   尝试使用 timm 在线预训练权重...")
            try:
                self.img_convnext = timm.create_model('convnext_tiny', pretrained=True)
                print("✅ 使用 timm ImageNet-1K 预训练权重")
                weights_loaded = True
            except Exception as e:
                print(f"❌ 在线下载也失败: {e}")
        ...
        # 4. 配置参数冻结策略
        print("配置模型参数冻结策略...")
        total_params = sum(p.numel() for p in self.img_convnext.parameters())

        # 冻结大部分参数
        for param in self.img_convnext.parameters():
            param.requires_grad = False

        # 解冻最后两个 stage（ConvNeXt 有 4 个 stage）
        # stages[-2] 和 stages[-1] 分别是倒数第二个和最后一个 stage
        if hasattr(self.img_convnext, 'stages'):
            for param in self.img_convnext.stages[-2].parameters():
                param.requires_grad = True
            for param in self.img_convnext.stages[-1].parameters():
                param.requires_grad = True
            print(f"✓ 解冻最后两个 stage")

        # 如果有 norm 层，也解冻
        if hasattr(self.img_convnext, 'norm'):
            for param in self.img_convnext.norm.parameters():
                param.requires_grad = True

        trainable_params = sum(p.numel() for p in self.img_convnext.parameters() if p.requires_grad)
        frozen_params = total_params - trainable_params

        print(f"✓ ConvNeXt-Tiny 参数配置:")
        ...
        # 5. 获取特征维度并替换分类头
        if hasattr(self.img_convnext, 'head'):
            if hasattr(self.img_convnext.head, 'fc'):
                num_ftrs = self.img_convnext.head.fc.in_features
                self.img_convnext.head.fc = nn.Identity()
            else:
                num_ftrs = self.img_convnext.head.in_features
                self.img_convnext.head = nn.Identity()
        else:
            num_ftrs = Config.feature_dim  # 使用默认值

        print(f"✓ ConvNeXt-Tiny 特征维度: {num_ftrs}")
        ...
        print(f"✓ NPY 特征提取器创建成功 (输出维度: 256)")
        ...
        print(f"  - 输入维度: {num_ftrs + 256} (图像 {num_ftrs} + NPY 256)")
    ...
    def forward(self, x):
        ...
        # ConvNeXt 前向传播
        img_feat = self.img_convnext(img)
...
def train_model():
    ...
    # 创建模型
    model = ImprovedMultimodalConvNeXt(
        len(Config.classes),
        pretrained_path=Config.local_convnext_pretrained_path
    ).to(Config.device)
    ...
    optimizer = torch.optim.AdamW([
        {'params': [p for p in model.img_convnext.parameters() if p.requires_grad],
    ...
    csv_path = os.path.join(Config.output_dir, 'training_log_convnext_tiny.csv')
    history_df.to_csv(csv_path, index=False)
    print(f"Training log saved to: {csv_path}")

    # 绘制训练曲线
    plot_path = os.path.join(Config.output_dir, 'training_metrics_convnext_tiny.png')
