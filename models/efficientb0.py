...
class Config:
    ...
    image_size = (224, 224)  # EfficientNet-B0 默认输入尺寸也是 224x224
    batch_size = 32
    random_seed = 42
    max_npy_channels = 3
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    num_epochs = 100
    lr_pretrained = 2e-5  # EfficientNet 预训练层的学习率可以适当更低
    lr_new = 1e-3  # 新增层的学习率
    weight_decay = 1e-4
    feature_dim = 1280  # EfficientNet-B0 的输出特征维度 (分类器前的特征维度)
    patience = 20
    output_dir = '../results/result1/efficientnet_b0'  # 更新输出目录以反映本地预训练
    # --- Path to your local pre-trained EfficientNet-B0 model ---
    local_efficientnet_pretrained_path = '../pre_models/efficientnet_b0.pth'
...
def load_efficientnet_pretrained_weights(model, weight_path):
    """
    专门用于加载 EfficientNet-B0 的 .bin 格式预训练权重
    支持自动处理键名不匹配和不同 checkpoint 格式

    Args:
        model: torchvision 的 EfficientNet-B0 模型实例
        weight_path: .bin 权重文件路径

    Returns:
        model: 加载权重后的模型
    """
    print(f"\n{'=' * 100}")
    print(f"正在加载 EfficientNet-B0 预训练权重...")
    print(f"权重文件路径: {weight_path}")
    print(f"{'=' * 100}")

    if not os.path.exists(weight_path):
        print(f"⚠️  警告：权重文件不存在，将使用随机初始化")
        return model
    ...
    try:
        ...
        if match_ratio >= 95:
            ...
            print(f"   1. 检查权重文件是否是 EfficientNet-B0")
            print(f"   2. 确认权重来源（torchvision/timm/其他）")
...
# Improved Multimodal EfficientNet Model
class ImprovedMultimodalEfficientNet(nn.Module):
    def __init__(self, num_classes, pretrained_path=None):
        super().__init__()

        # 1. 创建 EfficientNet-B0 模型（不加载预训练权重）
        print("创建 EfficientNet-B0 模型...")
        self.img_efficientnet = models.efficientnet_b0(weights=None)
        print("✓ 模型结构创建成功")

        # 2. 加载本地预训练权重
        if pretrained_path and os.path.exists(pretrained_path):
            self.img_efficientnet = load_efficientnet_pretrained_weights(
                self.img_efficientnet,
                pretrained_path
            )
        else:
            print(f"\n{'=' * 100}")
            print("⚠️  警告：本地预训练权重未找到")
            print(f"   路径: {pretrained_path}")
            print("   尝试使用 torchvision 在线预训练权重...")
            try:
                self.img_efficientnet = models.efficientnet_b0(
        ...
        # 3. 冻结大部分参数
        print("配置模型参数冻结策略...")
        for param in self.img_efficientnet.parameters():
            param.requires_grad = False

        # 4. 解冻最后两个 block
        # EfficientNet 的特征提取部分在 `features` 中
        for param in self.img_efficientnet.features[-2].parameters():  # 倒数第二个 block
            param.requires_grad = True
        for param in self.img_efficientnet.features[-1].parameters():  # 最后一个 block
            param.requires_grad = True

        # 统计可训练参数
        trainable_params = sum(p.numel() for p in self.img_efficientnet.parameters() if p.requires_grad)
        total_params = sum(p.numel() for p in self.img_efficientnet.parameters())
        print(f"✓ EfficientNet 参数配置:")
        print(f"  - 总参数: {total_params:,}")
        print(f"  - 可训练参数: {trainable_params:,} ({trainable_params / total_params * 100:.1f}%)")
        print(f"  - 冻结参数: {total_params - trainable_params:,}")

        # 5. 替换分类头为 Identity
        num_ftrs_img = Config.feature_dim
        self.img_efficientnet.classifier = nn.Identity()

        # 6. NPY Feature Extractor
        ...
        self.classifier = nn.Sequential(
            nn.Linear(Config.feature_dim + Config.feature_dim, 512),  # 1280 (img) + 1280 (npy) = 2560
            nn.BatchNorm1d(512),
            nn.ReLU(),
            nn.Dropout(0.5),
        ...
        print(f"  - 输入维度: {Config.feature_dim * 2} (图像 {Config.feature_dim} + NPY {Config.feature_dim})")
        print(f"  - 输出类别数: {num_classes}\n")

    def forward(self, x):
        img, npy = x
        img_feat = self.img_efficientnet(img)
...
def train_model():
    ...
    # 创建模型并传入本地预训练路径
    model = ImprovedMultimodalEfficientNet(
        len(Config.classes),
        pretrained_path=Config.local_efficientnet_pretrained_path
    ).to(Config.device)
    ...
    optimizer = torch.optim.AdamW([
        {'params': [p for name, p in model.img_efficientnet.named_parameters() if p.requires_grad],
         'lr': Config.lr_pretrained, 'weight_decay': Config.weight_decay},
        {'params': model.npy_feature_extractor.parameters(), 'lr': Config.lr_new,
    ...
    csv_path = os.path.join(Config.output_dir, 'training_log_efficientnetb0.csv')
    history_df.to_csv(csv_path, index=False)
    print(f"Training log saved to: {csv_path}")

    plot_path = os.path.join(Config.output_dir, 'training_metrics_efficientnetb0.png')
